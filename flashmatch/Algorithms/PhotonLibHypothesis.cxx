...
namespace flashmatch {
  ...
  void PhotonLibHypothesis::BuildHypothesis(const QCluster_t& trk, Flash_t &flash) const
  {
    ...
    #endif
    {
        ...
        for(size_t ipt = start_pt; ipt<num_pts; ++ipt) {
            ...
            for(size_t ipmt=0; ipmt < n_pmt; ++ipmt) {
                ...
                if(_global_qe_refl > 0.)
                    q0 = pt.q * DetectorSpecs::GetME().GetVisibilityReflected(vox_id,ipmt);

                if(!_uncoated_pmt_list[ipmt])
                    q1 = pt.q * DetectorSpecs::GetME().GetVisibility(vox_id, ipmt);

                local_pe_v[ipmt] += q1;
                local_pe_refl_v[ipmt] += q0;
        ...
        {
            for(size_t ipmt = 0; ipmt < n_pmt; ++ipmt) {
                flash.pe_v[ipmt] += (local_pe_v[ipmt] * _global_qe / _qe_v[ipmt]) + (local_pe_refl_v[ipmt] * _global_qe_refl * _qe_v[ipmt]);
            }
        }
    }
    return;
